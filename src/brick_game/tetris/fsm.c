...
void userInput(UserAction_t signal_, bool hold, Parameters_t *parameters_) {
  if (*parameters_->current_state_ >= NUM_STATES && signal_ >= NUM_ACTIONS)
    return;
  ActionCallback action = parameters_->fsm_->action_table_[*parameters_->current_state_][signal_];
...
void LaunchEventStat(Parameters_t *parameters_, EventStat_t event_) {
  if (parameters_->fsm_->event_table_[*(parameters_->current_state_)][event_] != NULL) {
    parameters_->fsm_->event_table_[*(parameters_->current_state_)][event_](parameters_);
  }
}
